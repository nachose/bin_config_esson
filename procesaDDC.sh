...
mkdir  $HOME/ddc/process
rm $HOME/ddc/process/*
#echo Descomprimiendo
cd  $HOME/ddc/process
for i in $runningTest/Pc*/ddc/diag*.tar.gz ; do
...
$HOME/bin/dataPrueba.sh $runningTest > /tmp/dataprueba.txt 

jq -r -f command.jq *$kpiPattern* >>  output
awk -F"," '{OFS=",";  if (NR>1){ $(NF+1)=strftime("%Y-%m-%d %H:%M:%S", $1)} ; print $0}'  output > /tmp/outputDdc.txt 
#awk -F"," '{OFS=",";  if (NR>1){ $(NF+1)=strftime("%Y-%m-%d %H:%M:%S", $1)} ; print $0}'  output > fullddc$dateex.csv 
paste /tmp/outputDdc.txt /tmp/dataprueba.txt> $HOME/results/fullddc$dateex.csv 
